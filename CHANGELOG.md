...
- Append-only Urlaubskontobuchungen mit Art, Begründung und optionalem
  Verfallsdatum.
- Eigenes Recht `Vacation.TeamCalendar`: berechtigte Beschäftigte erreichen
  den Teamkalender direkt im normalen Urlaubsbereich.
- Grafische Monats-/Wochenansicht mit Feiertagen, Listenalternative und
  Überschneidungshinweis beim Stellen eines Antrags.
- Automatischer, idempotenter Jahresübertrag bei aktiviertem Übertrag.
...
- Bestehende Benutzer ohne Arbeitszeitplan behalten exakt die bisherige
  Mo–Fr-Berechnung.
- Die Wochenarbeitszeit wird bei vorhandenem Plan aus den sieben Tageswerten
  abgeleitet; die Planverwaltung liegt in einem Modal mit Historie.
...
- Unterschiedlich lange Arbeitstage werden als einzelne Urlaubstage gezählt.
...
- Neue Spalte `vacation_requests.absence_type_key VARCHAR(64) NOT NULL DEFAULT
  'vacation'`.
- `vacation_entitlement_entries.source_key VARCHAR(191) NULL` samt Unique-Index.
...
- Logische Backups nehmen die neuen Metadatentabellen automatisch auf; bestehende
  ...
  23 auf.
- Migration 24 ergänzt den Quellschlüssel portabel und idempotent.
