...
def test_calendar_and_private_ics_feed(client):
    ...
    try:
        ...
        db.close()
    page = client.get("/records/vacations/calendar?month=2026-07&view=list")
...
def test_calendar_is_graphical_and_work_schedule_is_a_modal(client):
    _login(client)
    calendar = client.get("/records/vacations/calendar?month=2026-07")
    assert 'class="vacation-calendar-grid"' in calendar.text
    user_form = client.get("/admin/users/1").text
    assert 'id="work-schedule-modal"' in user_form
    assert 'id="work-schedule-open"' in user_form


def test_vacation_days_do_not_divide_by_one_daily_target(client):
    from app import database, models, services
    db = database.SessionLocal()
    try:
        user = db.get(models.User, 1)
        user.annual_vacation_days = 30
        user.work_schedules[:] = [models.WorkSchedule(
            user_id=1, valid_from=date(2026, 1, 1), monday_minutes=480,
            tuesday_minutes=240, wednesday_minutes=0, thursday_minutes=0,
            friday_minutes=0, saturday_minutes=0, sunday_minutes=0,
        )]
        vacations = [models.VacationRequest(user_id=1, start_date=date(2026, 7, 6),
            end_date=date(2026, 7, 7), status=models.VacationStatus.APPROVED,
            absence_type_key="vacation")]
        assert services.calculate_vacation_summary(user, vacations, 2026).used_days == 2.0
    finally:
        db.close()


def test_automatic_carryover_is_idempotent(client):
    from app import database, models, services
    db = database.SessionLocal()
    try:
        user = db.get(models.User, 1)
        user.vacation_carryover_enabled = True
        user.annual_vacation_days = 30
        db.commit()
        assert services.ensure_annual_carryovers(db, 2027) == 1
        assert services.ensure_annual_carryovers(db, 2027) == 0
        entries = db.query(models.VacationEntitlementEntry).filter_by(user_id=1, year=2027, kind="carryover").all()
        assert len(entries) == 1 and entries[0].days == 30
    finally:
        db.close()
