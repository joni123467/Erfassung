...
- **PWA-Installierbarkeit** über Manifest (`/static/manifest.webmanifest`) und Service Worker (`/static/sw.js`). Die installierte App startet über `/mobile`; bei Offline-Navigation liefert der Service Worker eine lokale Fallback-Shell (`/static/offline.html`), damit die App ohne Serververbindung weiter nutzbar bleibt.
