...
def ensure_schema() -> None:
    ...
    with database.engine.begin() as connection:
        ...
        if "vacation_entitlement_entries" in table_names:
            columns = {column["name"] for column in inspector.get_columns("vacation_entitlement_entries")}
            if "source_key" not in columns:
                connection.execute(text(
                    "ALTER TABLE vacation_entitlement_entries ADD COLUMN source_key VARCHAR(191)"
                ))
            connection.execute(text(
                "CREATE UNIQUE INDEX IF NOT EXISTS uq_vacation_entitlement_source_key "
                "ON vacation_entitlement_entries(source_key)"
            ))
...
def ensure_seed_data():
    ...
    with database.SessionLocal() as carryover_db:
        created = services.ensure_annual_carryovers(carryover_db, date.today().year)
        if created:
            logging_setup.log_application(
                f"Automatischer Urlaubsübertrag: {created} Anspruchsbuchung(en) angelegt"
            )
...
def records_vacations_page(request: Request, db: Session = Depends(database.get_db)):
    ...
    if can_team_calendar:
        ...
        query = db.query(models.VacationRequest).filter(
            ...
            models.VacationRequest.start_date <= date(today.year + 1, 12, 31),
        )
        if allowed is not None:
            query = query.filter(models.VacationRequest.user_id.in_(allowed or {-1}))
        team_conflicts = [{"start": item.start_date.isoformat(), "end": item.end_date.isoformat(), "person": item.user.full_name}
                          for item in query.all() if item.user_id != user.id]
    return templates.TemplateResponse(
        "records/vacations.html",
        {
            ...
            "can_request_vacations": _can_request_vacations(user),
            "can_team_calendar": can_team_calendar, "team_conflicts": team_conflicts,
...
def personal_vacation_calendar(
    request: Request, month: Optional[str] = None, scope: str = "self", view: str = "month",
    db: Session = Depends(database.get_db),
):
    user = get_logged_in_user(request, db)
    if not user:
        return RedirectResponse(url="/login", status_code=303)
    selected, start, end = _resolve_month_period(month)
    can_team = permission_service.has(user, "Vacation.TeamCalendar")
    scope = "team" if scope == "team" and can_team else "self"
    allowed_ids: Optional[set[int]] = {user.id}
    if scope == "team":
        allowed_ids = permission_service.allowed_user_ids(db, user, "Vacation.TeamCalendar")
    query = db.query(models.VacationRequest).filter(
        models.VacationRequest.start_date <= end,
        models.VacationRequest.end_date >= start,
    ...
    vacations = query.order_by(models.VacationRequest.start_date).all()
    can_manage = permission_service.has(user, "Vacation.Manage")
    if scope == "team" and not can_manage:
        vacations = [item for item in vacations if item.status == models.VacationStatus.APPROVED or item.user_id == user.id]
    holiday_rows = db.query(models.Holiday).filter(models.Holiday.date >= start, models.Holiday.date <= end).all()
    holidays_by_day = {item.date: item.name for item in holiday_rows}
    events_by_day: dict[date, list[dict]] = {}
    for item in vacations:
        current = max(item.start_date, start)
        while current <= min(item.end_date, end):
            events_by_day.setdefault(current, []).append({
                "person": item.user.full_name if scope == "team" else "",
                "label": "Abwesend" if scope == "team" else _calendar_rows(db, [item])[0]["label"],
                "status": item.status,
                "half": (current == item.start_date and item.half_day_start) or (current == item.end_date and item.half_day_end),
            })
            current += timedelta(days=1)
    weeks = []
    for week in monthcalendar(selected.year, selected.month):
        weeks.append([
            None if number == 0 else {
                "date": date(selected.year, selected.month, number), "number": number,
                "events": events_by_day.get(date(selected.year, selected.month, number), []),
                "holiday": holidays_by_day.get(date(selected.year, selected.month, number)),
            }
            for number in week
        ])
    calendar_view = view if view in {"month", "week", "list"} else "month"
    display_weeks = weeks
    if calendar_view == "week":
        reference_day = date.today() if date.today().year == selected.year and date.today().month == selected.month else start
        display_weeks = [next((week for week in weeks if any(cell and cell["date"] == reference_day for cell in week)), weeks[0])]
    return templates.TemplateResponse("records/vacation_calendar.html", {
        "request": request, "user": user, "selected_month": selected, "month_value": f"{selected.year:04d}-{selected.month:02d}",
        "weeks": display_weeks, "calendar_rows": _calendar_rows(db, vacations, reveal=scope == "self"),
        "calendar_scope": scope, "calendar_view": calendar_view,
        "feed_url": request.query_params.get("feed_url"), "can_sync": licensing.has_feature("calendar_sync"),
        "can_team_feed": can_team, "can_team_calendar": can_team,
...
def team_vacation_calendar(request: Request, db: Session = Depends(database.get_db)):
    actor = get_logged_in_user(request, db)
    if not actor:
        return RedirectResponse(url="/login", status_code=303)
    if not permission_service.has(actor, "Vacation.TeamCalendar"):
        raise HTTPException(status_code=403)
    query = urlencode({"scope": "team", **dict(request.query_params)})
    return RedirectResponse(url=f"/records/vacations/calendar?{query}", status_code=303)
...
def admin_users_edit(request: Request, user_id: int, db: Session = Depends(database.get_db)):
    ...
    return _admin_template(
        ...
        now_year=date.today().year,
        derived_weekly_hours=services.weekly_target_minutes_for_date(target, date.today()) / 60,
    )
...
def add_work_schedule(
    ...
    duplicate = db.query(models.WorkSchedule).filter(
    ...
    following = db.query(models.WorkSchedule).filter(
        models.WorkSchedule.user_id == user_id,
        models.WorkSchedule.valid_from > valid_from,
    ).order_by(models.WorkSchedule.valid_from).first()
    plan = models.WorkSchedule(user_id=user_id, valid_from=valid_from,
    ...
    if target and valid_from <= date.today() and (plan.valid_until is None or plan.valid_until >= date.today()):
        target.standard_weekly_hours = round(sum(hours), 2)
    try:
        db.commit()
    except IntegrityError:
        db.rollback()
